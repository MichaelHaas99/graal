...
public final class LoadFieldNode extends AccessFieldNode
    ...
    public static ConstantNode asConstant(ConstantFieldProvider constantFields, ConstantReflectionProvider constantReflection,
        ...
        if (InlineTypeUtil.unproxify(object, tool) instanceof InlineTypeNode inlineTypeNode) {
            ...
            if (index != -1) {
                if (!StampTool.isPointerNonNull(object())) {
                    LogicNode check = IntegerEqualsNode.create(ConstantNode.forInt(1), inlineTypeNode.getNonNull(), NodeView.DEFAULT);
                    FixedGuardNode guard = graph().addOrUniqueWithInputs(new FixedGuardNode(check, DeoptimizationReason.NullCheckException, DeoptimizationAction.InvalidateReprofile, false));
